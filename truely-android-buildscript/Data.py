...
src_path = cur_path + '/truely_src_code'  # 代码路径
flutter_src_path = cur_path + '/local-flutter'  # 代码路径
...
mapping_src = '%s/app/build/outputs/mapping/release/mapping.txt'
...
apk_path = '%s/app/build/outputs/apk/%s/%s'
metadata_path ='%s/app/build/outputs/apk/%s/output-metadata.json'
web_target_dir_unsigned = '/var/wwwroot/android/release/%s/%s/%s%s'  # 服务器存档路径
web_location_dir_unsigned = 'http://dailybuild.yidian-inc.com/android/release/%s/%s/%s%s'
web_terminal = 'android-builder@dailybuild.yidian-inc.com'
